...
\author{
  ...
  Vicente Alves Cordeiro Junior\\[0.4cm]
...
\noindent Para evitar sobreposição visual, o modelo foi dividido em três diagramas menores: identidade e agenda, registros clínicos e automações de apoio.
...
\node[entity] (usuarios) {\textbf{usuarios}\nodepart{second}PK id\_usuario\\nome, email, cpf\\senha\_hash\\tipo\_usuario};
\node[entity, below left=1.3cm and 1.2cm of usuarios] (clientes) {\textbf{clientes}\nodepart{second}PK id\_cliente\\FK id\_usuario\\telefone, endereço\\observações};
\node[entity, below right=1.3cm and 1.2cm of usuarios] (veterinarios) {\textbf{veterinarios}\nodepart{second}PK id\_veterinario\\FK id\_usuario\\crmv, especialidade\\telefone, ativo};
\node[entity, below=1.5cm of clientes] (animais) {\textbf{animais}\nodepart{second}PK id\_animal\\FK id\_cliente\\nome, espécie, raça\\sexo, peso, ativo};
\node[entity, below=1.5cm of veterinarios] (procedimentos) {\textbf{procedimentos}\nodepart{second}PK id\_procedimento\\nome, tipo\\duração padrão\\preço base};
\node[entity, below right=1.5cm and 0.2cm of animais] (atendimentos) {\textbf{atendimentos}\nodepart{second}PK id\_atendimento\\FK animal, cliente\\FK veterinário\\FK procedimento\\início, fim, status\\valor total};
\node[entity, right=1.3cm of veterinarios] (bloqueios) {\textbf{bloqueios\_agenda}\nodepart{second}PK id\_bloqueio\\FK veterinário opcional\\início, fim\\motivo, criado\_por};
...
\caption{Modelo principal: usuários, perfis, animais, procedimentos e agenda}
\end{figure}
...
\node[entity] (animais) {\textbf{animais}\nodepart{second}PK id\_animal\\FK id\_cliente\\nome, espécie\\peso atual};
\node[entity, right=1.7cm of animais] (atendimentos) {\textbf{atendimentos}\nodepart{second}PK id\_atendimento\\FK animal\\FK veterinário\\FK procedimento\\início, status};
\node[entity, right=1.7cm of atendimentos] (veterinarios) {\textbf{veterinarios}\nodepart{second}PK id\_veterinario\\FK id\_usuario\\crmv\\especialidade};
\node[entity, below=1.4cm of atendimentos] (prontuarios) {\textbf{prontuarios}\nodepart{second}PK id\_prontuario\\FK atendimento\\FK animal\\FK veterinário\\diagnóstico\\prescrição};
\node[entity, below=1.4cm of animais] (pesos) {\textbf{historico\_pesos}\nodepart{second}PK id\_peso\\FK animal\\FK veterinário\\peso, data};
\node[entity, below=1.4cm of veterinarios] (vacinas) {\textbf{vacinas\_aplicadas}\nodepart{second}PK id\_vacina\\FK animal\\FK atendimento\\FK procedimento\\próxima dose};
\draw[rel] (atendimentos) -- node[note]{N:1} (animais);
\draw[rel] (atendimentos) -- node[note]{N:1} (veterinarios);
\draw[rel] (prontuarios) -- node[note]{1:1} (atendimentos);
\draw[rel] (prontuarios) -- node[note]{N:1} (animais);
\draw[rel] (prontuarios) -- node[note]{N:1} (veterinarios);
...
\caption{Modelo clínico: prontuários, vacinas e histórico de pesos}
\end{figure}
...
\node[entity] (procedimentos) {\textbf{procedimentos}\nodepart{second}PK id\_procedimento\\nome, tipo\\duração padrão\\preço base};
\node[entity, right=1.7cm of procedimentos] (precos) {\textbf{historico\_precos}\nodepart{second}PK id\_historico\\FK procedimento\\preço anterior/novo\\alterado\_por};
\node[entity, below=1.4cm of procedimentos] (esquemas) {\textbf{esquemas\_vacinais}\nodepart{second}PK id\_esquema\\nome, espécie\\descrição};
\node[entity, right=1.7cm of esquemas] (etapas) {\textbf{etapas\_esquema}\nodepart{second}PK id\_etapa\\FK esquema\\FK procedimento\\intervalo, repetição};
\node[entity, below=1.4cm of esquemas] (lembretes) {\textbf{lembretes}\nodepart{second}PK id\_lembrete\\FK animal, cliente\\FK atendimento\\tipo, prioridade\\status, data prevista};
\node[entity, right=1.7cm of lembretes] (contatos) {\textbf{contatos\_lembrete}\nodepart{second}PK id\_contato\\FK lembrete\\realizado\_por\\canal, resultado};
...
\draw[rel] (contatos) -- node[note]{N:1} (lembretes);
\end{tikzpicture}
}
\caption{Modelo de apoio: preços, esquemas vacinais, etapas e lembretes}
\end{figure}
...
\subsection{Stored Procedure e Function com Parâmetro}

No PostgreSQL, o sistema utiliza uma procedure real e também funções armazenadas. A procedure \texttt{atualizar\_status\_lembrete} recebe o identificador do lembrete, o novo status e o usuário responsável. Ela é chamada pelo backend com \texttt{CALL}, atendendo diretamente ao requisito de stored procedure com passagem de parâmetro.

\begin{lstlisting}[style=codigo,language=SQL,caption={Procedure armazenada com parâmetros}]
...
\begin{lstlisting}[style=codigo,language=JavaScript,caption={Chamada da procedure pelo backend}]
db.query(
...
Além da procedure, foi utilizada uma função armazenada em PL/pgSQL. Ela recebe o identificador do animal como parâmetro e retorna seu histórico clínico.
...
Além do histórico clínico, as buscas principais do sistema também foram movidas para funções armazenadas no PostgreSQL. Dessa forma, o backend deixa de montar consultas extensas para listagens e relatórios, passando apenas os parâmetros de perfil do usuário e recebendo o resultado já filtrado pelo banco.

\begin{lstlisting}[style=codigo,language=JavaScript,caption={Chamadas de buscas implementadas como funções armazenadas}]
db.query(
...
db.query("SELECT * FROM relatorio_servicos()");
\end{lstlisting}
...
\texttt{atualizar\_status\_lembrete(id, status, usuario)} & Procedure real chamada com \texttt{CALL} para atualizar lembretes. \\
\texttt{listar\_clientes()} & Lista tutores com total de animais por meio de junções. \\
\texttt{listar\_animais(tipo, cliente)} & Lista animais e aplica filtro automático para o perfil tutor. \\
\texttt{listar\_atendimentos(tipo, cliente, veterinario)} & Retorna a agenda filtrada conforme o perfil logado. \\
\texttt{listar\_prontuarios(tipo, cliente, veterinario)} & Retorna prontuários visíveis conforme permissão do perfil. \\
\texttt{listar\_lembretes(tipo, cliente, veterinario)} & Retorna lembretes filtrados por administrador, veterinário ou tutor. \\
\texttt{relatorio\_servicos()} & Agrupa atendimentos por procedimento para relatório. \\
\texttt{relatorio\_veterinarios()} & Agrupa atendimentos por veterinário para relatório administrativo. \\
